...
from application.matcher_service import match_documents
...
async def match(req: MatchRequest) -> List[MatchResponse]:
    domain_keywords = to_domain_keywords(req.keywords)
    matched_docs = await match_documents(domain_keywords, req.language_code)
